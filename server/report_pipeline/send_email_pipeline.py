...
def get_latest_report(topic: str):
    if not os.path.exists(report_dir):
        return None

    topic_safe = topic.replace(' ', '_')

    search_dir = os.path.join(report_dir, topic_safe)
...
def send_email(topic: str):
    if not SENDER or not PASSWORD or not RECEIVER:
        print("Email sender, password, or receiver is not set")
        return
    
    attachment_path = get_latest_report(topic)
    ...
    msg['From'] = SENDER
    msg['To'] = RECEIVER
    msg['Subject'] = f"Weekly Research Report - {topic} - {datetime.today().strftime('%Y-%m-%d')}"
    ...
    if attachment_path:
        ...
        Here is your weekly AI research report generated by your Agent for the topic: {topic}.
...
if __name__ == "__main__":
    send_email(topic="minimal surface")